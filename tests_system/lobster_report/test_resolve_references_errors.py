...
class ReportResolveReferencesErrorsTest(LobsterReportSystemTestCaseBase):
    ...
    def test_unknown_tracing_target_yaml_no_schema(self):
        ...
        self._test_runner.declare_input_file(
            self._data_directory / "unknown_tracing_target_no_schema.yaml"
        )
        self._test_runner.declare_input_file(
            self._data_directory / "python_unknown_tracing_target_no_schema.lobster"
        )

        self._test_runner.cmd_args.lobster_config = (
            "unknown_tracing_target_no_schema.yaml"
        )
        self._test_runner.cmd_args.out = (
    ...
    def test_tracing_destination_unversioned_yaml_no_schema(self):
        ...
        # lobster-trace: core_report_req.Tracing_Destination_Unversioned
        self._test_runner.declare_input_file(
            self._data_directory / "unversioned_trace_no_schema.yaml"
        )
        self._test_runner.declare_input_file(
            self._data_directory / "python_unversioned_trace_dest_no_schema.lobster"
        )
    ...
    def test_tracing_destination_version_mismatch_yaml_no_schema(self):
        ...
        # lobster-trace: core_report_req.Tracing_Destination_Version_Mismatch
        self._test_runner.declare_input_file(
            self._data_directory / "version_mismatch_trace_no_schema.yaml"
        )
        self._test_runner.declare_input_file(
            self._data_directory / "python_ver_mismatch_trace_dest_no_schema.lobster"
        )
