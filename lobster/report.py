        ...
        if "custom_data" in data:
            ...
            for key, value in custom_data.items():
                if not isinstance(key, str):
                    self.mh.error(loc, f"Key in 'custom_data' must be a string, got {type(key).__name__}.")
                if not isinstance(value, str):
                    self.mh.error(loc, f"Value for key '{key}' in 'custom_data' must be a string, got {type(value).__name__}.")
