...
from lobster.common.items import Implementation, Item, Tracing_Tag, KindTypes
...
class ItemBuilder:
    ...
    def __init__(self, kind: str = KindTypes.ITM.value) -> None:
        self.kind = kind
    ...
    def from_match(self, match: Match) -> Item:
        """
        Generate an Item or Implementation object from a regex match object.
        """
        ...
        if self.kind == KindTypes.IMP.value:
            item = Implementation(
                tag = self._get_tag(filename, function_name, line_nr),
                location = self._get_location(filename, line_nr),
                language = "C/C++",
