...
class LOBSTER_Tool(SupportedCommonConfigKeys, metaclass=ABCMeta):
    ...
    def process_commandline_options(
        ...
        options.inputs_from_file = self.config.get("inputs_from_file")
        options.inputs = self.config.get("inputs", [])
        options.traverse_bazel_dirs = self.config.get("traverse_bazel_dirs", False)
        options.single = self.config.get("single", False)
