...
def main():
    ...
    if len(discovered_tests) != len(declared_tests):
        print(
            ...
            "and bazel py_test target count."
        )
    if missing_build_dirs:
        print("Subfolders with tests but without BUILD.bazel:")
        for tool_name in missing_build_dirs:
            print(f"  - {tool_name}")
    if missing_test_files:
        print("Test files missing in BUILD.bazel file:")
        for test_path in missing_test_files:
            print(f"  - {test_path}")
    if stale_test_entries:
        print("py_test(srcs) entries whose test file is missing:")
