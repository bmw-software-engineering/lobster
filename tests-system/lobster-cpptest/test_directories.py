...
class DirectoriesCpptestTest(LobsterCpptestSystemTestCaseBase):
    ...
    def test_all_files_from_current_directory_consumed_cpptest(self):
        ...
        self.OUT_FILE = "nested_directories.lobster"
        ...
        asserter.assertStdOutNumAndFile(101, self.OUT_FILE)
        asserter.assertExitCode(0)
        asserter.assertOutputFiles()

    def test_files_from_specified_directory_consumed_cpptest(self):
        # lobster-trace: Usecases.Incorrect_Number_of_Cpp_Tests_in_Output
        self.OUT_FILE = "specific_directory.lobster"
        self._test_runner.cmd_args.out = self.OUT_FILE
        ...
        asserter = Asserter(self, completed_process, self._test_runner)
        asserter.assertNoStdErrText()
        asserter.assertStdOutNumAndFile(30, self.OUT_FILE)
        asserter.assertExitCode(0)
        asserter.assertOutputFiles()

    def test_specified_directory_and_files_consumed_cpptest(self):
        # lobster-trace: Usecases.Incorrect_Number_of_Cpp_Tests_in_Output
        self.OUT_FILE = "directory_files.lobster"
        self._test_runner.cmd_args.out = self.OUT_FILE
        ...
        asserter = Asserter(self, completed_process, self._test_runner)
        asserter.assertNoStdErrText()
        asserter.assertStdOutNumAndFile(43, self.OUT_FILE)
        asserter.assertExitCode(0)
        asserter.assertOutputFiles()

    def test_no_cpptest_file(self):
        self.OUT_FILE = "no_cpptest_file.lobster"
        self._test_runner.cmd_args.out = self.OUT_FILE
