...
- Group 1: levels `A` and `B`, combined into one group by a
  ...
  references `X`).
- Group 2: level `C`, which traces to `X` without being combined by any
...
```mermaid
graph LR
    subgraph Group1["Required down reference group 1"]
        A[Level A]
        B[Level B]
    end

    subgraph Group2["Required down reference group 2"]
        C[Level C]
    end
