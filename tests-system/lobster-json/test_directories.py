...
class InputDirectoryJsonTest(LobsterJsonSystemTestCaseBase):
    ...
    def test_missing_directory(self):
        ...
        asserter.assertStdOutText(
            f"<config>: lobster error: {dir_does_not_exist} is not a file or directory\n"
        )
        asserter.assertExitCode(1)

    def test_empty_directory(self):
        # lobster-trace: json_req.Input_Empty_Directory
        OUT_FILE = "empty.lobster"
        self._test_runner.cmd_args.out = OUT_FILE
        self._test_runner.declare_output_file(self._data_directory / OUT_FILE)

        empty_dir_name = "empty-directory"
    ...
    def test_consumes_files_in_specified_directory(self):
        # lobster-trace: json_req.Input_Directory_Consumption
        OUT_FILE = "valid-directory.lobster"
        self._test_runner.cmd_args.out = OUT_FILE
        self._test_runner.config_file_data.tag_attribute = "tags"
        self._test_runner.config_file_data.name_attribute = "name"
        self._test_runner.config_file_data.inputs.append("valid-directory")

        source_dir = self._data_directory / "valid-directory"
        dest_dir = self._test_runner.working_dir / "valid-directory"
