...
from lobster.tools.cpp_parser.parser.builds_collector.domain_model.datatypes import PipelineType, ZuulJobConfiguration
...
class ConfigSingleton:
    def __init__(self):
        ...
        self.zuul_username = os.environ.get("ZUUL_USERNAME")
        self.zuul_password = os.environ.get("ZUUL_PASSWORD")
        self.builds_collector_num_last_builds = 20
        self.builds_collector_job_names = [
            ZuulJobConfiguration("adp-foresight-gcc9-opt-fast-test-stp", PipelineType.GATE),
        ]
        self.builds_collector_post_job_names = [
            ZuulJobConfiguration("ddad-all-gcc9-opt-slow-test-aws", PipelineType.POST_INDEPENDENT),
            ZuulJobConfiguration("ddad-all-gcc9-dbg-slow-test-aws", PipelineType.POST_INDEPENDENT),
        ]
        self.builds_collector_job_name = self.builds_collector_job_names[0]
        self.builds_collector_job_url = []
        self.builds_collector_credentials_file = None
    ...
    def _parse_args(self):
        ...
        parser.add_argument(
            "--zuul_username",
            default=self.zuul_username,
            type=str,
            help="Zuul API username, if not specified will try to use the envvar 'ZUUL_USERNAME'.",
        )
        parser.add_argument(
            "--zuul_password",
            default=self.zuul_password,
            type=str,
            help="Zuul API password, if not specified will try to use the envvar 'ZUUL_PASSWORD'.",
        )
        parser.add_argument(
            "--builds_collector_num_last_builds",
            default=self.builds_collector_num_last_builds,
            type=int,
            help="Number of recent builds.",
        )
        parser.add_argument(
            "--builds_collector_job_name",
            default=self.builds_collector_job_name,
            type=str,
            help="Select build type, for which the pipline should be run.",
            choices=self.builds_collector_job_names,
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--builds_collector_credentials_file",
            default=None,
            type=str,
            help="Path to file containing crta credentials.",
        )
        parser.add_argument(
    ...
    def init(self, parse_args=None):
        ...
        self._set_member("zuul_username")
        self._set_member("zuul_password")
        self._set_member("builds_collector_num_last_builds")
        self._set_member("builds_collector_job_name")
        self._set_member("builds_collector_credentials_file")
