...
class LobsterCodebeamerTest(LobsterCodebeamerSystemTestCaseBase):
    ...
    def test_retry_if_configured(self):
        ...
        self.assertEqual(
            "Fetching page 1 of query...\n0\n(RequestHistory(method='GET', url='/api/"
            "v3/reports/999/items?page=1&pageSize=100', error=None, status=429,"
            " redirect_location=None), RequestHistory(method='GET',"
            " url='/api/v3/reports/999/items?page=1&pageSize=100',"
            " error=None, status=429, redirect_location=None))\n"
            "Could not fetch "
