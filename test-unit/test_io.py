...
class TestLobsterWrite(unittest.TestCase):
    ...
    def test_lobster_read_valid_req(self, mock_from_json, mock_key, mock_additional_data_from_json):
    ...
    @patch("lobster.items.Tracing_Tag.key")
    @patch("lobster.items.Tracing_Tag.from_json")
    def test_lobster_read_valid_imp(self, mock_from_json, mock_key, mock_additional_data_from_json):
        mock_key.return_value = "mock_namespace mock_tag"
        mock_from_json.return_value = self.tracing_tag
        self.source_data.update({"location" : {
                    "kind": "file",
    ...
    @patch("lobster.items.Tracing_Tag.key")
    @patch("lobster.items.Tracing_Tag.from_json")
    def test_lobster_read_valid_act(self, mock_from_json, mock_key, mock_additional_data_from_json):
        mock_key.return_value = "mock_namespace mock_tag"
        mock_from_json.return_value = self.tracing_tag
        self.source_data.update({"location" : {
                    "kind": "file",
