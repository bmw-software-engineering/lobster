...
`misplaced_tag_in_value_node.pkg`: Only the first-level `<TsBlock>`
under `<TESTSTEPS>`, and its direct `<TsBlock>` children,
are allowed to contain lobster-trace: tags.
...
`misplaced_tag_in_analysis_node.pkg`:
Line 12 of this file is the case.
There is a warning when a `<DESCRIPTION>` with lobster-trace exists 
under `<TRACE-ANALYSIS>`, such as:
